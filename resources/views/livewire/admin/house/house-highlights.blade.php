<div class="mt-4">
    <div class="sm:border-b border-gray-200 sm:pb-5 sm:flex sm:items-center sm:justify-between">
        <h3 class="text-base font-semibold leading-6 text-gray-900">House highlights</h3>
        ...
        <div class="bg-cool-gray-200 p-4 rounded shadow-inner flex relative">
            <div class="w-1/2 pr-2 space-y-4">
                <div class="flex flex-row space-x-2">
                    <div class="flex flex-row space-x-2">
                        <div class="basis-1/2">
                            <x-input.group  for="filter-publication-id" label="Highlight id">
            ...
            <x-slot name="body">
                ...
                <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                    <x-table.cell colspan="7">
                        @unless ($selectAll)
                        <div>
                            <span>You have selected <strong>{{ $highlights->count() }}</strong> highlights, do you want to select all <strong>{{ $highlights->total() }}</strong>?</span>
                            <x-button.link wire:click="selectAll" class="ml-1 text-blue-600">Select All</x-button.link>
                        </div>
                        @else
                        <span>You are currently selecting all <strong>{{ $highlights->total() }}</strong> highlights.</span>
                        @endif
                    </x-table.cell>
                </x-table.row>
                @endif

                @forelse ($highlights as $row)
                ...
                <x-table.row>
                    <x-table.cell colspan="8">
                        <div class="flex justify-center items-center space-x-2">
                            ...
                            <span class="font-medium py-8 text-cool-gray-400 text-xl">No highlight found...</span>
                        </div>
                    </x-table.cell>
                </x-table.row>
                @endforelse
    ...
    <form wire:submit.prevent="save">
        <x-modal.dialog wire:model.defer="showEditModal">
            <x-slot name="title">Edit or create a highlight</x-slot>
