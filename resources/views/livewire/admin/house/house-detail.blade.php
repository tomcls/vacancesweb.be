      ...
      <div class="space-y-10 sm:space-y-10">
        ...
        <div class="sm:grid sm:grid-cols-5   sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:py-5  ">
          ...
          <div class="flex flex-col sm:flex-row justify-between sm:space-x-2 col-span-3 sm:col-span-5 lg:col-span-3">
            ...
            <div class="justify-self-start basis-3/5">
              <x-input.group  for="user-search" label="User" :error="$errors->first('house.user_id')">
                  <x-input.autocomplete 
                      wire:model="userSearch" 
                      wire:keyup="usersResult"
                      id="user-search" 
                      name="user-search" 
                      wireModel="house.user_id"
                      :rows="$users" 
        ...
        <div>
          @foreach ($titles as $key => $title)
              @livewire('admin.house.house-title',['houseTitle' => $title,'lang'=>$lang,'errors'=>$errors],key('title-'.$key))
          @endforeach
        </div>
        <div class="sm:grid sm:grid-cols-3 border-t-gray-500 space-x-10">
          <div class="space-y-8" >
            ...
            <div class="flex flex-row space-x-2">
              <x-input.group   label="Longitude" for="longitude" :error="$errors->first('house.longitude')">
                <x-input.text wire:model.debounce.150ms="house.longitude" id="longitude"  >
                  <x-slot:leadingIcon>
                      <x-icon.map class="flex-shrink-0 text-sky-500"/>
                  </x-slot:leadingIcon>
                </x-input.text>
              </x-input.group>
              <x-input.group   label="Latitude" for="latitude" :error="$errors->first('house.latitude')">
                <x-input.text wire:model="house.latitude" id="latitude"  >
                  <x-slot:leadingIcon>
                      <x-icon.map class="flex-shrink-0 text-sky-500"/>
                  </x-slot:leadingIcon>
                </x-input.text>
        ...
        <div>
          @foreach ($descriptions as $key => $description)
              @livewire('admin.house.house-description',['houseDescription' => $description,'lang'=>$lang],key('description-'.$key))
          @endforeach
        </div>
      </div>
    </form>
