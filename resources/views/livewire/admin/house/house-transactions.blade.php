<div class="mt-4">
    <div class="sm:border-b border-gray-200 sm:pb-5 sm:flex sm:items-center sm:justify-between">
        <h3 class="text-base font-semibold leading-6 text-gray-900">Invoice transactions</h3>
        ...
        <x-modal.dialog wire:model.defer="showEditModal">
            ...
            <x-slot name="content">
                <x-input.group for="invoice_id" label="Invoice Id" :error="$errors->first('editing.invoice_id')">
                    <x-input.text wire:model.lazy="editing.invoice_id" id="invoice_id" placeholder="if invoiceId empty, a new invoice is created with this transaction" />
                </x-input.group>
               
                <x-input.group for="type" label="Transaction type" :error="$errors->first('editing.type')">
                    <x-input.select  wire:model.lazy="editing.type" >
                        <option value=""></option>
                        @foreach (\App\Data\Enums\InvoiceTransactionTypeEnum::cases() as $unit)
                            @if ($unit != \App\Data\Enums\InvoiceTransactionTypeEnum::HolidayPurchase && $unit != \App\Data\Enums\InvoiceTransactionTypeEnum::HousePackage)
                            <option value="{{ $unit }}">{{$unit }}</option>
                            @endif
                        @endforeach
                    </x-input.select>
                ...
                <x-input.group for="house_publications" label="Reference" :error="$errors->first('editing.reference')" class="{{($editing->type==\App\Data\Enums\InvoiceTransactionTypeEnum::HousePublication)?'':'hidden'}}">
                    <x-input.select  wire:model.lazy="editing.publication"  id="house_publications">
                        <option value=""></option>
                        @foreach ($housePublications as $pub)
                            <option value="{{ $pub->id }}">{{'publication_id: '.$pub->id.'# from '.$pub->startdate .' to '.$pub->enddate }}</option>
                        @endforeach
                    </x-input.select>
                </x-input.group>
                ...
                <x-input.group for="house_highlights" label="Reference" :error="$errors->first('editing.reference')" class="{{($editing->type==\App\Data\Enums\InvoiceTransactionTypeEnum::HouseHighlight)?'':'hidden'}}">
                    <x-input.select  wire:model.lazy="editing.highlight"  id="house_highlights">
                        <option value=""></option>
                ...
                <x-input.group for="house_packages" label="Reference" :error="$errors->first('editing.reference')" class="{{($editing->type==\App\Data\Enums\InvoiceTransactionTypeEnum::HousePackage)?'':'hidden'}}">
                    <x-input.select  wire:model.lazy="editing.package"  id="house_packages">
                        <option value=""></option>
                ...
                <x-input.group for="price" label="Transaction price" :error="$errors->first('editing.price')">
                    <x-input.text wire:model.lazy="editing.price" id="price" placeholder="Price" />
                </x-input.group>
                <x-input.group for="datePayed" label="Date payed" :error="$errors->first('invoice.date_payed_for_editing')">
                    <x-input.date wire:model.lazy="invoice.date_payed_for_editing" id="datePayed" placeholder="Date payed" />
                </x-input.group>

                <x-input.group for="payment_status" label="Transaction status" :error="$errors->first('invoice.payment_status')">
                    <x-input.select  wire:model.lazy="invoice.payment_status"  id="payment_status">
                        @foreach (\App\Data\Enums\InvoicePaymentStatusEnum::cases() as $unit)
