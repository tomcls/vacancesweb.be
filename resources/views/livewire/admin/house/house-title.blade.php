<div >
    <x-input.group  inline label="Title" for="name_{{$houseTitle->lang}}" hidden="{{$lang!=$houseTitle->lang?1:0}}" :error="$errors->first('houseTitle.name')">
      ...
      <x-input.group  inline label="Slug" for="slug_{{$houseTitle->lang}}" hidden="{{$lang!=$houseTitle->lang?1:0}}" :error="$errors->first('houseTitle.slug')">
        <x-input.text 
            wire:model="houseTitle.slug"  
            x-ref="slug"
            id="slug_{{$houseTitle->lang}}"  >
            <x-slot:leadingIcon>
                <x-icon.edit class="flex-shrink-0 text-sky-500"/>
            </x-slot:leadingIcon>
        </x-input.text>
