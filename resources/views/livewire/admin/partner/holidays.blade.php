<div>
    <div class="sm:border-b border-gray-200 sm:pb-5 sm:flex sm:items-center sm:justify-between">
        <h3 class="text-base font-semibold leading-6 text-gray-900">Partner holidays</h3>
                ...
                <x-slot name="body">
                    ...
                    <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                        <x-table.cell colspan="5">
                            @unless ($selectAll)
                            <div>
                                <span>You have selected <strong>{{ $rows->count() }}</strong> partners, do you want to select all <strong>{{ $rows->total() }}</strong>?</span>
                                <x-button.link wire:click="selectAll" class="ml-1 text-blue-600">Select All</x-button.link>
                            </div>
                            @else
                            <span>You are currently selecting all <strong>{{ $rows->total() }}</strong> partners.</span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                    @endif
                    ...
                    <x-table.row>
                        <x-table.cell colspan="7">
                            <div class="flex justify-center items-center space-x-2">
                                ...
                                <span class="font-medium py-8 text-cool-gray-400 text-xl">No partner holiday found...</span>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                    @endforelse
    ...
    <form wire:submit.prevent="save">
        <x-modal.dialog wire:model.defer="showEditModal">
            {{$errors}}
            <x-slot name="title">Edit holiday item</x-slot>
    ...
    <form wire:submit.prevent="deleteSelected">
        <x-modal.confirmation wire:model.defer="showDeleteModal">
            <x-slot name="title">Delete partner holiday(s)</x-slot>
            <x-slot name="content">
