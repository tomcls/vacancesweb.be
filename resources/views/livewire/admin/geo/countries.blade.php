<div>
    ...
    <div class="sm:border-b border-gray-200 sm:pb-5 sm:flex sm:items-center sm:justify-between">
        <h3 class="text-base font-semibold leading-6 text-gray-900">Countries</h3>
        <div class="mt-3 sm:ml-4 sm:mt-0">
          ...
          <div class="flex rounded-md shadow-sm">
            <div class="relative flex-grow focus-within:z-10">
              ...
              <input type="text" name="mobile-search-candidate" id="mobile-search-candidate" wire:model="filters.search" placeholder="Search countries..." class="block w-full rounded-none rounded-l-md border-0 py-1.5 pl-10 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:hidden" placeholder="Search">
              <input type="text" name="desktop-search-candidate" id="desktop-search-candidate" wire:model="filters.search" placeholder="Search countries..." class="hidden w-full rounded-none rounded-l-md border-0 py-1.5 pl-10 text-sm leading-6 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:block" placeholder="Search candidates">
            </div>
        ...
        <div class="flex-col space-y-4">
            <x-table>
                <x-slot name="head">
                    ...
                    <x-table.heading  >Iso code</x-table.heading>
                    <x-table.heading  >Abbreviation</x-table.heading>
                    <x-table.heading  >Centroid</x-table.heading>
                    <x-table.heading  >Active</x-table.heading>
                    <x-table.heading  >total regions</x-table.heading>
                    <x-table.heading  >total holidays</x-table.heading>
                    <x-table.heading  >total houses</x-table.heading>
                ...
                <x-slot name="body">
                    ...
                    <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                        <x-table.cell colspan="7">
                            @unless ($selectAll)
                            <div>
                                <span>You have selected <strong>{{ $countries->count() }}</strong> countries, do you want to select all <strong>{{ $countries->total() }}</strong>?</span>
                                <x-button.link wire:click="selectAll" class="ml-1 text-blue-600">Select All</x-button.link>
                            </div>
                            @else
                            <span>You are currently selecting all <strong>{{ $countries->total() }}</strong> countries.</span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                    @endif
                    ...
                    <x-table.row wire:loading.class.delay="opacity-70" wire:key="row-{{ $row->id }}">
                        ...
                        <x-table.cell >
                            <a href="{{ route('admin.country').'/'.$row->id }}">{{ $row->id }}</a>
                        </x-table.cell>
                        <x-table.cell>
                            <a href="{{ route('admin.country').'/'.$row->id }}" class="inline-flex space-x-2 truncate text-sm leading-5">
                                <p class="text-cool-gray-600 truncate">
                                    {{ $row->translation($filters['lang'])->first()->name }}
                                </p>
                            </a>
                        </x-table.cell>
                        <x-table.cell>
                            <a href="{{ route('admin.country').'/'.$row->id }}" class="inline-flex space-x-2 truncate text-sm leading-5">
                                <p class="text-cool-gray-600 truncate">
                                    {{ $row->translation($filters['lang'])->first()->slug }}
                                </p>
                            </a>
                        </x-table.cell>
                        <x-table.cell>
                    ...
                    <x-table.row>
                        <x-table.cell colspan="7">
                            <div class="flex justify-center items-center space-x-2">
                                <x-icon.flag class="h-8 w-8 text-cool-gray-400" />
                                <span class="font-medium py-8 text-cool-gray-400 text-xl">No country found...</span>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                    @endforelse
