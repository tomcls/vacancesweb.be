    ...
    <div class="flex-col space-y-4">
        <x-table>
            <x-slot name="head">
                ...
                <x-table.heading sortable multi-column wire:click="sortBy('partner')" :direction="$sorts['partner'] ?? null" >Partner name</x-table.heading>
            ...
            <x-slot name="body">
                ...
                <x-table.row wire:loading.class.delay="opacity-70" wire:key="row-{{ $row->document->id }}">
                    ...
                    <x-table.cell >
                        <x-button.link wire:click="edit({{ $row->document->id }})">{{ $row->document->partner }}</x-button.link>
                    </x-table.cell>
                    <x-table.cell>
    ...
    <form wire:submit.prevent="save">
        <x-modal.dialog wire:model.defer="showEditModal">
            ...
            <x-slot name="content">
                ...
                <x-input.group for="partner" label="Partner" :error="$errors->first('editing.type')">
                    <x-input.select  wire:model="editing.partner" id="partner">
                        <option value=""></option>
